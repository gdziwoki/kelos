...
func newGetAgentConfigCommand(cfg *ClientConfig, allNamespaces *bool) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 1 {
				...
				default:
					if detail {
						...
						printAgentConfigTable(os.Stdout, []kelosv1alpha2.AgentConfig{*ac}, false)
					}
					return nil
				}
			}
			...
			acList.SetGroupVersionKind(kelosv1alpha2.GroupVersion.WithKind("AgentConfigList"))
			switch output {
			case "yaml":
