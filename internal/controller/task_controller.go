...
func (r *TaskReconciler) refreshGitHubAppTokenIfNeeded(ctx context.Context, task *kelosv1alpha1.Task) (time.Duration, error) {
	logger := log.FromContext(ctx)
	...
	if r.TokenClient == nil {
		return 0, fmt.Errorf("GitHub App token refresh requested but TokenClient is not configured")
	}
	...
	if err := r.Get(ctx, client.ObjectKey{Namespace: task.Namespace, Name: appSecretName}, &appSecret); err != nil {
		return 0, fmt.Errorf("fetching GitHub App secret %q: %w", appSecretName, err)
	}
	...
	tokenResp, err := tc.GenerateInstallationToken(ctx, creds)
	if err != nil {
		return 0, fmt.Errorf("generating refreshed installation token: %w", err)
	}
	...
	if err := r.Update(ctx, &tokenSecret); err != nil {
		return 0, fmt.Errorf("updating token secret with refreshed token: %w", err)
	}
